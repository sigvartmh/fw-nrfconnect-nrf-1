...
static int z_to_nrf_flags(int z_flags)
{
	 ...
	 * may also be missing from libmodem.
				       ...
				       socklen_t *addrlen)
{
	...
	if ((addr != NULL) && (addrlen != NULL)) {
		...
		/* Workaround for the libmodem issue, making `nrf_accept` to
					    ...
					    int flags)
{
	...
	static uint8_t buf[CONFIG_LIBMODEM_SENDMSG_BUF_SIZE];
...
static int nrf91_libmodem_socket_offload_init(const struct device *arg)
{
	ARG_UNUSED(arg);
...
NET_DEVICE_OFFLOAD_INIT(nrf91_socket, "nrf91_socket",
			nrf91_libmodem_socket_offload_init,
