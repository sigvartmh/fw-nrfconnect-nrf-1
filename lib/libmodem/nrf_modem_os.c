...
#ifdef CONFIG_LIBMODEM_TRACE_ENABLED
 ...
 * counter value at which libmodem last checked the 'readiness' of a thread
 */
 ...
 * meantime), or whether we should return to libmodem to re-verify if a sleep is
 * needed.
 */
...
void nrf_modem_os_errno_set(int err_code)
{
	...
	default:
		 ...
		 */
		__ASSERT(false, "Untranslated errno %d set by libmodem!", err_code);
		LOG_ERR("Untranslated errno %d set by libmodem!", err_code);
		errno = 0xBAADBAAD;
		break;
	}
}
