...
{
	u8_t start_block = (BSD_RESERVED_MEMORY_ADDRESS - 0x20000000)/SPU_RAMREGION_SIZE;
...
{
       NRF_SPU_S->PERIPHID[NRFX_PERIPHERAL_ID_GET(NRF_POWER_NS)].PERM = PERIPH_LOCK | PERIPH_NONSEC;
       NRF_SPU_S->PERIPHID[NRFX_PERIPHERAL_ID_GET(NRF_POWER_S)].PERM = 0;
}

static inline void setup_s_ipc(void)
{
        NRF_SPU_S->PERIPHID[NRFX_PERIPHERAL_ID_GET(NRF_IPC_S)].PERM = PERIPH_LOCK | PERIPH_NONSEC;
}
