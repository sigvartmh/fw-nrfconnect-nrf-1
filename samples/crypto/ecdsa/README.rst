...
The ECDA show how to sign/verify messages using SHA256 as the hashing algorithm and the secp256r1 curve.

It does the following things:

First, the sample signs a sample plaintext:
   #. The Platform Security Architecture (PSA) API is initialized.
   #. An ECDSA keypair is generated and imported to the PSA keystore.
   #. Signing is performed.
   #. The keypair is removed from the PSA keystore.

Then, the sample verifies the signature:
   #. The Platform Security Architecture (PSA) API is initialized
   #. The ECDSA keypair is imported to the PSA keystore.
   #. Verification of the signagure is performed.
   #. The keypair is removed from the PSA keystore
