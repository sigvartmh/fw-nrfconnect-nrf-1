...
.. option:: CLOUD_CERT_SEC_TAG - Security tag for TLS credentials
   ...
   If you used LTE Link Monitor to store the certificates, make sure to configure the security tag that you used to store them.
...
.. option:: USE_NRF_CLOUD - Use nRF Cloud certificates
...
.. option:: PROVISION_CERTIFICATES - Provision certificates from certificates.h file
   ...
   .. warning::
      * The sample will overwrite existing certificates that are stored with the configured security tag.
      * Provision the certificates only once and then uncheck this option to prevent your certificates from being visible in the application binary and modem logs, and to reduce flash tear.
