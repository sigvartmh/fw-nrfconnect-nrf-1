 ...
 *
 * This implements the fetching an AWS job which is queued when the
 * device connects to the AWS MQTT broker.
 *
 * @param[in] client           Initialized and connected MQTT Client instance.
 * @param[in] job_id           The ID of the job you are fetching use "$next"
 *		                       for fetching the next IN_PROGRESS.
 *		                       or QUEUED job. It will fetch the first
 *		                       created job if multiple exsists.
 *
 * @retval 0       If the update is published successfully, otherwise return
 *	               mqtt_publish error code or the error code of snprintf.
 */
