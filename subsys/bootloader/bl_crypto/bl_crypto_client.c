...
enum abi_index {ROT_VERIFY, BL_SHA256, BL_SECP256R1, LAST};
...
const struct fw_abi_info *get_bl_crypto_abi(enum abi_index abi)
{
	...
	if (!bl_crypto_abis[abi]) {
		switch(abi) {
		case ROT_VERIFY:
			bl_crypto_abis[abi] = abi_find(ROT_VERIFY_ABI_ID, ROT_VERIFY_ABI_FLAGS,
					ROT_VERIFY_ABI_VER, ROT_VERIFY_ABI_MAX_VER);
			break;
		case BL_SHA256:
			bl_crypto_abis[abi] = abi_find(BL_SHA256_ABI_ID, BL_SHA256_ABI_FLAGS,
					BL_SHA256_ABI_VER, BL_SHA256_ABI_MAX_VER);
			break;
		case BL_SECP256R1:
			bl_crypto_abis[abi] = abi_find(BL_SECP256R1_ABI_ID, BL_SECP256R1_ABI_FLAGS,
...
const struct rot_verify_abi *get_rot_verify_abi(void)
{
	return (const struct rot_verify_abi *)get_bl_crypto_abi(ROT_VERIFY);
}
...
{
	return get_rot_verify_abi()->abi.root_of_trust_verify(public_key,
