				...
				const char *client_token)
{
		...
		printk("Lib: %s, ret:%d, max:%d", status_details, ret,
		       STATUS_DETAILS_MAX_LEN);
				      ...
				      char *file_path_buf)
{
	...
	printk("\n\rLib job_id: %s\n", job_id);
	printk("Lib hostname: %s\n", hostname);
	printk("Lib file_path: %s\n", file_path);
	printk("Lib doc_version: %d\n", job.execution.version_number);
	doc_version_number = job.execution.version_number;
	...
	if (!strncmp(notify_next_topic, topic,
		...
		if (execution_state == AWS_JOBS_IN_PROGRESS &&
			...
			/* callback_emit(AWS_FOTA_EVT_FINISHED); */
			    ...
			    MIN(JOB_ID_UPDATE_TOPIC_MAX_LEN, topic_len))) {
		/* emit aws_fota failure our update to the job document was rejected */
	}
	return 0;
...
static void http_fota_handler(enum fota_download_evt_id evt)
{
	...
	case FOTA_DOWNLOAD_EVT_FINISHED:
		...
		update_job_execution(c, job_id, AWS_JOBS_SUCCEEDED,
				     fota_state, doc_version_number, "");
		/* Emit AWS_FOTA_DONE when update_job_execution is done*/
