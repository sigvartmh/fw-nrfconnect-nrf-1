 ...
 */
static inline void report_snprintf_err(u32_t ret, u32_t size, char *entry)
{
	if (ret >= size) {
 ...
 */
static void construct_get_topic(u8_t *client_id, struct mqtt_topic *topic)
{
 ...
 * @param[out] topic Outputs the constructed mqtt_topic into the given buffer.
 */
static void construct_notify_next_topic(u8_t *client_id,
...
int aws_jobs_subscribe_notify_next(struct mqtt_client *client)
{
	...
	construct_notify_next_topic(client->client_id.utf8, &subscribe_topic);
 ...
 * @param[out] topic Outputs the constructed mqtt_topic into the given buffer.
 *
 * @retval mqtt_topic Returns formated mqtt_topic.
 */
static void construct_notify_topic(u8_t *client_id, struct mqtt_topic *topic)
{
...
{
	struct mqtt_topic subscribe_topic;
	construct_notify_topic(client->client_id.utf8, &subscribe_topic);
				       ...
				       bool notify_next)
{
	...
	construct_get_topic(client_id, subscribe_topics[0]);
	if (notify_next) {
		construct_notify_next_topic(client_id, subscribe_topics[1]);
	} else {
		construct_notify_topic(client_id, subscribe_topics[1]);
	}
 ...
 * @param[out] topic Outputs the constructed mqtt_topic into the given buffer.
 */
static void construct_job_id_get_topic(u8_t *client_id,
				       u8_t *job_id,
				       struct mqtt_topic *topic)
{
 ...
 * @param[out] topic Outputs the constructed mqtt_topic into the given buffer.
 */
static void construct_job_id_update_topic(u8_t *client_id,
				          u8_t *job_id,
				          struct mqtt_topic *topic)
{
